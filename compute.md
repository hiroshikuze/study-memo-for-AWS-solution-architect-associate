...
|No|種類|内容|
|--:|:---|:---|
|1|スポット|AWS側で余裕があるときに利用できる、最大**90%**の割引<br>中断されることがあるので、アプリケーションはEC2に障害が発生しても適切に対処できる必要がある<br>中断の2分前に通知が来る|
|2|リザーブド|予約することで最大**72%**の割引を受ける<br>いつでもは使えない|
|3|オンデマンド|利用した分だけ課金|
|4|dedicated host|インスタンスを完全に一つのアカウント専用に使用する物理サーバー|

### [リザーブドインスタンス](https://aws.amazon.com/jp/ec2/pricing/reserved-instances/)

利用期間分をあらかじめ購入することで最大**72%**の割引価格が適応

|種類|内容|
|:---|:---|
|Standardタイプ|リージョンやAZを指定してインスタンスを購入、同じインスタンス構成なら同じリージョンやAZ内でインスタンスの配置変更が可能、インスタンスファミリー／OS／テナンシー／支払オプションの変更ができない|
|Convertibleタイプ|自由に構成変更が可能だが、Standardタイプより割高、最大**66%**の割引|
|スケジュール|1年間に渡り毎日・毎週・毎月キャパシー予約を受けつける（**2021年にサービス終了**）|
...
Elastic IPアドレスやパブリックIPアドレスはVPC内の他のEC2インスタンスからのアクセスには使用されない  
EC2を再起動する度にパブリックIPは変更されるので、外部から特定のIPアドレスでアクセスできるようにするにはElastic IPの固定が必要  
...
|セキュリティグループ|EC2などに用いる、ファイアウォールで外部アクセスに対応<br>デフォルトではすべての同じセキュリティグループに割り当てられたインスタンスからのアクセスのみ許可している|
