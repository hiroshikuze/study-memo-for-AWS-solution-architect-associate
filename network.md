...
|Geolocation|クライアントの位置情報（国・大陸）に基づく|
|Geoproximity|クライアントとリソースの地理的距離に基づく（Traffic Flowが必要）|
|IPベース（IP-based）|クライアントのIPアドレスのCIDRブロックに基づく（2022年追加）|
|マルチバリュー|複数の値をランダムに返す、ヘルスチェック対応|
...
パブリックサブネット＝トラフィックがインターネットゲートウェイにルーティングされている  
プライベートサブネット＝インターネットゲートウェイへのルートがない  

## CIDR

[クラスレスアドレッシング](http://e-words.jp/w/CIDR.html)ブロック:許可・拒否するネットワークアドレスを表記「192.168.0.0/16」のように記述、すでに使用されて設定できないアドレスもある
